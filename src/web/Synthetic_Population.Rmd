...
The above picture is a screenshot from Iterative Propoertional Fitting result. When we take a look at it, we can see that there are four variables. They are Race, Tenure, Age and Salary.


Basically it shows the number of householders for different conditions of variables. For example, if we see the second row of the first column, the value is **3**. It indicates us that there 3 householders who are White with salary less than 10,000, betwwen age 25 - 44  and owning a home.

This picture is a small portion from  IPF output.The actual output contains values for various conditions of variables. 
...
We have created a Dataframe from the IPF output and the dataframe is linked below. We can find the frequency of various householders in that dataframe.
...
- Final step of  involves attaching household geometry to the synthetic population. For that process we took the blackknight housing data, performed shape file conversion to extract the latitiudes and longitudes coordinates  and linked the coordinates with their unique housing IDs to our IPF outputs. 

###**Northampton and Accomack Synthetic Population**
The dataframes for Synthetic population of Northampton and Accomack counties are linked below.
